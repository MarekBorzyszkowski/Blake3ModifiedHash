...
@jit(nb.types.UniTuple(nb.uint16, 2)(nb.uint16[:]), nopython=True, parallel=True)
def crack_hash(expected_hash):
    number_of_elements = len(allowed_val)
    combinations = []
    result = [np.uint16(0), np.uint16(0)]
...
original_combination = crack_hash(
