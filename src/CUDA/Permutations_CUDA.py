...
@cuda.jit(nb.void(nb.uint32, nb.uint32), device=True)
def rotl(n, d):
    n = ((n << d) | (n >> SHORT_SIZE - d)) & 0xFFFF


@cuda.jit(nb.void(nb.uint32, nb.uint32, nb.uint32, nb.uint32, nb.uint32, nb.uint32),
          device=True)
...
@cuda.jit(nb.void(nb.uint32[:]), device=True)
